...
} from "../../constants/keys/application.key";
import { REQUEST, APPLICATION_DETAIL } from "../../types/application.type";

export const ApplicationDetailFeature = (
  request?: REQUEST,
  ...
  const { data } = useQuery<APPLICATION_DETAIL>(
    [location ? GET_APPLICATION_RESULT : GET_APPLICATION_DETAIL],
    () => {
